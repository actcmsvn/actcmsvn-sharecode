...
namespace Actcmsvn\Sharecode\Traits;
...
trait AppBoot
{
    ...
    public function isLegal()
    {
        ...
        $data = [
            ...
            'email' => '',
            'domain' => $domain,
            'itemId' => config('actcmsvn_sharecode.sharecode_item_id'),
            'appUrl' => urlencode(url()->full()),
            'version' => $version,
        ];

        // worksuite, worksuite-saas, recruit-saas, recruit, appointo
        $companiesEmailArray = ['23263417','20201709','24061995','22336912','22989501'];
    ...
    {
        return view('vendor.actcmsvn_sharecode.verify_purchase');
    }
    ...
    public function curl($postData)
    {
        ...
        try {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, config('actcmsvn_sharecode.verify_url'));
            ...
            return [
                'status' => 'success',
                'messages' => 'Mã mua hàng của bạn đã được xác minh thành công'
            ];
        }
    ...
    private function getServerData($purchaseCode, $savePurchaseCode = true)
    {
        ...
        if ($response['status'] === 'success') {
            ...
            return Reply::successWithData($response['message'] . ' <a href="' . route(config('actcmsvn_sharecode.redirectRoute')) . '">Bấm để quay lại</a>', ['server' => $response]);
        }
    ...
    public function showInstall()
    {
        try {
            ...
            echo view('vendor.actcmsvn-sharecode.install_message');
            exit(1);
        }
    }
    ...
    public function curlReviewContent($buttonPressedType)
    {
        // Verify purchase
        try {
            $url = str_replace('verify-purchase','button-pressed',config('actcmsvn_sharecode.verify_url'));
    ...
    public function isCheckScript()
    {
        ...
        if (is_null($this->appSetting->purchase_code)) {
            $data = [
                ...
                'email' => '',
                'domain' => $domain,
                'itemId' => config('actcmsvn_sharecodeo.sharecode_item_id'),
                'appUrl' => urlencode(url()->full()),
                'version' => $version,
            ];
