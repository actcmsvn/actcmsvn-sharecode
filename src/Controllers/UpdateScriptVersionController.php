...
namespace Actcmsvn\Sharecode\Controllers;

use Actcmsvn\Sharecode\Helpers\Reply;
...
class UpdateScriptVersionController extends Controller
{
    ...
    {
        return config('actcmsvn_sharecode.allow_users_id');
    }
    ...
    */
    public function update()
    {
        ...
        try {
            ...
            $filename_tmp = config('actcmsvn_sharecode.tmp_path') . '/' . $update_name;
            ...
            $this->configClear();
            return Reply::successWithData('Bắt đầu tải về...', ['description' => $lastVersionInfo['description']]);
    ...
    public function install()
    {
        ...
        $update_path = config('actcmsvn_sharecode.tmp_path') . '/' . $archive;
        ...
        $this->clean();
        return Reply::success('Đã giải nén thành công. Đang cài đặt...');
    }
    ...
    */
    public function download(Request $request)
    {
        ...
        $downloadRemoteUrl = config('actcmsvn_sharecode.update_baseurl') . '/' . $update_name;
        ...
        return Reply::success('Đã tải xong. Đang cài đặt...');
    ...
    */
    ...
    private function restore()
    {
        ...
        try {
            ...
            echo "<BR>[ THất bại ]";
            echo "<BR> Thư mục sao lưu nằm trong: <i>" . $backup_dir . "</i>.";
    ...
    public function checkIfFileExtracted()
    {
        ...
        if ($lastVersionInfo['version'] == $this->getCurrentVersion()) {
            ...
            Auth::logout();
            return Reply::success('Cài đặt thành công.');
        }
    }
    ...
    public function updateDatabase()
    {
        ...
        return 'Đã cập nhật cơ sở dữ liệu thành công. <a href="' . route(config('actcmsvn_sharecode.redirectRoute')) . '">Nhấn vào đây để đăng nhập</a>';
    }
    ...
    public function clearCache()
    {
        $this->configClear();
        if (request()->ajax()) {
            return Reply::success('Đã xóa bộ nhớ cache thành công.');
        }

        return 'Đã xóa bộ nhớ cache thành công. <a href="' . route(config('actcmsvn_sharecode.redirectRoute')) . '">Nhấn vào đây để đăng nhập</a>';
    }
    ...
    public function refreshCache()
    {
        ...
        if (request()->ajax()) {
            return Reply::success('Đã làm mới bộ nhớ cache thành công.');
        }

        return 'Đã làm mới bộ nhớ cache thành công. <a href="' . route(config('actcmsvn_sharecode.redirectRoute')) . '">Nhấn vào đây để đăng nhập</a>';
    }
}
