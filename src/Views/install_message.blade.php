<html>
<head>
    <title>{{ ucwords(config('actcmsvn_sharecode.sharecode_product_name'))}} Not installed</title>
...
<div class="container">
    <div class="row" style="margin-top: 30px">
        ...
        <div class="bs-example" data-example-id="alerts-with-links" style="margin-top:10px ">
            ...
            <div class="alert alert-warning" role="alert"><strong>{{ ucwords(config('actcmsvn_sharecode.sharecode_product_name'))}} chưa cài đặt!</strong> Ghé  <a
                    href="{{ url('/install')}}"
                    class="alert-link">{{ url('/install')}}</a>
                để tải trình cài đặt.
            </div>
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title">Yêu cầu máy chủ.
                    @if (version_compare(PHP_VERSION, '7.1.3') > 0)
                        <span class="pull-right">Phiên bản PHP hiện tại: {{ phpversion() }} <i
                                class="fa fa fa-check-circle text-success"></i></span>
                    @else
                        <span class="pull-right">Phiên bản PHP hiện tại: {{ phpversion() }} <i data-toggle="tooltip"
